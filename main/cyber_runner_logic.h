// main/cyber_runner_logic.h —— 《霓虹疾行：影刃闪现》(Cyber Courier: Phantom Dash) 核心物理与算法引擎
// 专为 ESP32-C3 极简三键(UP/DOWN/OK)与零动态堆分配(Zero malloc)设计。
// 特性：多段建筑断崖流式生成、空中幽灵闪现虚化穿透、二段跳跃、贴地滑铲/急降俯冲、
// 脉冲激光、浮游无人机穿爆、易碎天窗与超导排风弹射。
#pragma once
...
#define CR_PLAYER_X          44    // 玩家在屏幕上的基准横向锚点
...
#define CR_MAX_PARTICLES     28    // 动态微粒池
...
typedef enum {
    ...
    CR_STANCE_BLINK,         // 空中幽灵闪现 (相位虚化无敌突进)
    CR_STANCE_SLIDE,         // 地面贴地滑铲 (火花四溅钻低位)
    CR_STANCE_DIVE,          // 空中极速俯冲 (下坠砸地)
...
typedef enum {
    ...
    CR_HAZARD_DRONE,         // 浮游巡逻无人机 (可规避或闪现穿爆)
...
typedef enum {
    CR_PART_NONE = 0,
    CR_PART_SPARK,           // 滑铲/碰撞金属火花 (抛物线)
    CR_PART_NEON_BURST,      // 闪现虚化/晶体拾取霓虹微粒 (辐射扩散)
    CR_PART_EXPLOSION,       // 无人机被穿爆机械碎片
...
typedef enum {
    ...
    CR_SND_SLIDE,            // 贴地滑铲高频切削摩擦音
    CR_SND_DIVE_SLAM,        // 俯冲砸地震撼冲击
    CR_SND_GEM,              // 拾取数据晶体双音和弦
    CR_SND_DRONE_POP,        // 闪现穿爆无人机炸裂音
    CR_SND_VENT_BOOST,       // 排风口离子暴风喷射音
    CR_SND_HURT,             // 受创警报音
