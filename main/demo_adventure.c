    ...
    if (s_game.player.is_alive) {
        ...
        if (show_player) {
            ...
            if (s_game.player.has_skateboard) {
                draw_box(layer, px - 2, py + 22, 20, 4, 0xEF4444);
                draw_box(layer, px, py + 25, 4, 3, 0xFFFFFF);
                draw_box(layer, px + 12, py + 25, 4, 3, 0xFFFFFF);
                // 疾速冲刺粒子
                if ((s_frame_tick % 2) == 0) {
                    draw_box(layer, px - 6, py + 23, 4, 2, 0xFBBF24);
                }
            }
            ...
            if (s_game.player.on_ground) {
                ...
                draw_box(layer, px + 3, py + 20, 4, 4, 0x78350F); // 腾空
            }
        }
    }
}
