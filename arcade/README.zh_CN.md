...
AI Passport 游戏合集的公开游乐场。大厅里十台柜机，休息室还有一只世界钟。每款都沿用掌机的三颗键：UP、DOWN、OK。
...
simulator/              可玩页面
  ...
  games-impl.js         游戏实现
  thunder.html          雷霆战机独立桌面
  flydriver.html
  flysaber.html
  hub.html              较早的多标签桌面
```

把 `arcade/index.html` 和旁边的 `simulator/` 一起打开（本地静态服务，或把 GitHub Pages 指到仓库根目录）。柜机链接指向 `../simulator/game.html?id=...`。
